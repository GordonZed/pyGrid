...
body=[((grid.width/2),(grid.width/2)+1),((grid.width/2),(grid.width/2)),((grid.width/2),(grid.width/2)-1)]
...
while done == False:
	...
	if idle <> True: #following line prevents flickering when idle
		...
		if ate_apple == False: #del last segment unless apple is eaten, this effectively adds a body segment
			grid.off(body[0][0],body[0][1])
			del body[0]
